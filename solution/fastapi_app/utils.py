...
def count_files(directory):
    ...
    :param directory: Path to the directory to count files in.
...
# Напиши, что можно использовать различные балансировщики. В том числе основанные на исторических данных, 
# а текущая реализация это базовый алгоритм
# также можно учитывать объем задачи (количество текста или длительность аудио) и исходя из этого выделять ядра
def balancer_n_cores(files_count, all_files_count):
    n_cores = int((files_count / all_files_count) * N_CORES_AVAILABLE)
    return n_cores if n_cores > 0 else N_CORES_DEFAULT

def get_n_cores(files_count, all_files_count):
    if BALANCER:
        return balancer_n_cores(files_count, all_files_count)
    else:
...
def get_files(dirs):
    full_path_files = []
    for dir_ in dirs:
        full_path_files.extend(glob.glob(dir_ + "*")) 
    return full_path_files


def send_to_storage(path: str = 'local'):
    if path['type'] == 'local':
        path = path['path']
        if not os.path.exists(path):
            os.makedirs(path)
            segment_path = os.path.join(path, f".txt")
            # segment.export(segment_path, format="wav")
            # print(f"Segment {i} saved: {segment_path}")
